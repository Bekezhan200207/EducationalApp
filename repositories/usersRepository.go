...
func (r *UsersRepository) Create(c context.Context, user models.User) (uuid.UUID, error) {
	...
	var id uuid.UUID
	err := r.db.QueryRow(c, "insert into users(user_name, user_surname, role_id, email, password_hash) values($1, $2, $3, $4, $5) returning uuid", user.Name, user.Surname, user.Role_id, user.Email, user.PasswordHash).Scan(&id)
	if err != nil {
...
func (r *UsersRepository) FindByUUID(c context.Context, strUUID string) (models.User, error) {
	logger := logger.GetLogger()
	...
	row := r.db.QueryRow(c, "select uuid, user_name, user_surname, role_id, email, password_hash from users where uuid = $1", parsed_UUID)
	err = row.Scan(&user.UUID, &user.Name, &user.Surname, &user.Role_id, &user.Email, &user.PasswordHash)
...
func (r *UsersRepository) FindAll(c context.Context) ([]models.User, error) {
	...
	rows, err := r.db.Query(c, "select uuid, user_name, user_surname, role_id, email, password_hash from users")
	if err != nil {
		logger.Error("could not query database", zap.String("db_msg", err.Error()))
		return nil, err
	}
	...
	for rows.Next() {
		var user models.User
		err := rows.Scan(&user.UUID, &user.Name, &user.Surname, &user.Role_id, &user.Email, &user.PasswordHash)
		if err != nil {
			logger.Error("could not scan query row", zap.String("db_msg", err.Error()))
			return nil, err
		}
...
func (r *UsersRepository) Update(c context.Context, user models.User, strUUID string) error {
	...
	_, err = r.db.Exec(c, "update users set user_name = $1, user_surname =$2 where uuid = $3", user.Name, user.Surname, parsed_UUID)
	if err != nil {
...
func (u *UsersRepository) FindByEmail(c context.Context, email string) (models.User, error) {
	...
	var user models.User
	row := u.db.QueryRow(c, "select uuid, user_name, user_surname, role_id, email, password_hash from users where email = $1", email)
	err := row.Scan(&user.UUID, &user.Name, &user.Surname, &user.Role_id, &user.Email, &user.PasswordHash)
